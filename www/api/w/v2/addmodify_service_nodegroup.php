...
if(count($details) != 1) {
	...
	if($ops->isYesNo($_POST['create_only'], false)) {
		print($ops->formatWriteOutput('400', 'Service/Nodegroup already exists'));
		exit(0);
	}

	$post_query = sprintf(" WHERE `service_id` = '%d'", $service_details['service_id']);
	$post_query .= sprintf(" AND `nodegroup` = '%s'", $nodegroup_details['nodegroup']);
	$query = 'UPDATE';
	$s_message = 'updated';
}
...
$query .= ' `service_nodegroups` SET ' . implode(',', $query_opts);
$query .= $post_query;
...
list($new_details, $junk) = $mon->listServicesNodegroups(array(
	'service_id' => $service_details['service_id'],
	'nodegroup' => $nodegroup_details['nodegroup'],
));
...
$message = sprintf("%s/%s %s", $new_details['services.service_name'], $new_details['nodegroups.nodegroup'], $s_message);
